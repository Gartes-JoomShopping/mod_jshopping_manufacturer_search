...
/**
 * @var $displayManufacturers array
 */
$displayManufacturers = $displayManufacturers ;

# разобрать массив по первым буквам
$ManufacturerAllLetters = [] ;
foreach ($displayManufacturers as $item)
{
    $firstLetter = mb_substr($item->name, 0, 1);
    $ManufacturerAllLetters[$firstLetter][] = $item;
...
<div cdkaccordionitem="" class="sidebar-block" data-filter-name="producer">
    <button class="sidebar-block__toggle" type="button">
        <span class="sidebar-block__toggle-title">Бренд
            <span class="sidebar-block__toggle-quantity"> <?= count( $displayManufacturers ) ?> </span>
        </span>
                    ...
                    <div rzdynamicfilterholder="">
                        <rz-filter-section-checkbox _nghost-c124="">
                            <ul _ngcontent-c124="" class="checkbox-filter">
                                <?php
                                foreach ($ManufacturerAllLetters as $fLetter => $manufacturerAllLetter)
                                {
                                    ?>
                                <li _ngcontent-c124="" class="checkbox-filter__item">
                                    ...
                                    <ul _ngcontent-c124="" class="checkbox-filter">
                                        <?php
                                        foreach ($manufacturerAllLetter as $item)
                                        {
                                            $inputClass = '';
                                            $inputState = '';
                                            ...
                                            <li _ngcontent-c124="" class="checkbox-filter__item">
                                                <a _ngcontent-c124="" class="checkbox-filter__link"
                                                   href="">
                                                    <input id="uf_manufacturer_<?php echo $item->id ?>" type="checkbox" name="manufacturer_id[]"
                                                           value="<?php echo $item->id ?>" <?php echo $inputState ?>
                                                           onclick="manufacturerSearch.updateForm(this)"/>
                                                    <label _ngcontent-c124="" for="ALDI"><?=$item->name?>
                                                        <span _ngcontent-c124=""
                                                              class="sidebar-block__quantity"><?=$item->count?></span>
                                                    </label>
                                                </a>
                                            </li>
                                            <?php
